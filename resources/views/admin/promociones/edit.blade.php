...
<h1>EDITAR PROMOCIÓN / {{ strtoupper($promocion->nombre) }}</h1>

<form action="{{ route('promociones.update', $promocion->id) }}" method="POST" class="main-form">
    @csrf
    @method('PUT')
    ...
    <div class="field">
        ...
        <input type="text" inputmode="decimal" name="valor" value="{{ old('valor', $promocion->valor) }}" required>
    ...
    <div class="field">
        ...
        <input type="date" name="fecha_inicio" value="{{ old('fecha_inicio', $promocion->fecha_inicio) }}" required>
        @error('fecha_inicio') <span class="error-msg">{{ $message }}</span> @enderror
    </div>

    <div class="field">
        <label>Fecha fin <span class="required-mark">(*)</span></label>
        <input type="date" name="fecha_fin" value="{{ old('fecha_fin', $promocion->fecha_fin) }}" required>
    ...
    <div class="field">
        ...
        <input type="text" name="codigo" value="{{ old('codigo', $promocion->codigo) }}" placeholder="Ej: VERANO25">
        @error('codigo') <span class="error-msg">{{ $message }}</span> @enderror
    </div>

    <div class="field">
        <label>Compra mínima ($)</label>
        <input type="text" inputmode="decimal" name="minimo_compra" value="{{ old('minimo_compra', $promocion->minimo_compra) }}" placeholder="Vacío = sin mínimo">
    ...
    <div class="field" style="justify-content: flex-end; padding-bottom: 4px;">
        ...
        <div class="checkbox-row">
            <input type="checkbox" name="activa" id="activa" {{ old('activa', $promocion->activa) ? 'checked' : '' }}>
    ...
    <div class="field full-width">
        ...
        <select name="aplica_a" id="aplica_a" required onchange="toggleAplicaA(this.value)">
            <option value="todo"     {{ old('aplica_a', $promocion->aplica_a) === 'todo'     ? 'selected' : '' }}>Todo el catálogo</option>
            <option value="marca"    {{ old('aplica_a', $promocion->aplica_a) === 'marca'    ? 'selected' : '' }}>Una marca específica</option>
            <option value="tipo"     {{ old('aplica_a', $promocion->aplica_a) === 'tipo'     ? 'selected' : '' }}>Un tipo de botín</option>
            <option value="producto" {{ old('aplica_a', $promocion->aplica_a) === 'producto' ? 'selected' : '' }}>Un producto específico</option>
        </select>
    ...
    <div class="field full-width hidden" id="field-marca">
        ...
        <select name="marca_id">
            ...
            @foreach($marcas as $marca)
                <option value="{{ $marca->id }}" {{ old('marca_id', $promocion->marca_id) === $marca->id ? 'selected' : '' }}>
                    {{ $marca->descripcion }}
                </option>
            @endforeach
        </select>
    ...
    <div class="field full-width hidden" id="field-tipo">
        ...
        <select name="tipo_producto">
            ...
            @foreach(\App\Models\Product::TIPOS as $tipo)
                <option value="{{ $tipo }}" {{ old('tipo_producto', $promocion->tipo_producto) === $tipo ? 'selected' : '' }}>
                    {{ $tipo }}
                </option>
            @endforeach
        </select>
