...
<h1>EDITAR MÉTODO / {{ strtoupper($metodo->descripcion) }}</h1>

<form action="{{ route('metodos_pago.update', $metodo->id) }}" method="POST" class="main-form">
    @csrf
    @method('PUT')

    <div class="field">
        <label>Descripción <span class="required-mark">(*)</span></label>
        <input type="text" name="descripcion" value="{{ old('descripcion', $metodo->descripcion) }}" required>
    ...
    <div class="field">
        ...
        <div class="checkbox-row">
            <input type="checkbox" name="activo" id="activo" {{ old('activo', $metodo->activo) ? 'checked' : '' }}>
